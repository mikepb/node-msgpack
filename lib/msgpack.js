...
for (var key in binding) {
  if (/^MSGPACK_/.test(key)) {
    exports[key] = binding[key];
  }
}
...
var pack = exports.pack = function(value, replacer, flags){
  var fn;
  if (~~flags !== flags) flags = 0;
  if (typeof replacer === 'function') fn = function(key, value){
    var self = this;
    if (key == null) {
      key = '';
      self = {};
    ...
    return fn.call(self, key, value);
  }
  return binding.pack(value, fn, flags);
};
...
Stream.prototype._onData = function(buf) {
  ...
  while (this.buf && this.buf.length > 0) {
    ...
    binding.offset = this.buf.length;
    // unpack
